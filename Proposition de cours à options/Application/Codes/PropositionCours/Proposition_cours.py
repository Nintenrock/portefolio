...
def interet_match_obj_des(cours, centre_interets, option_obligatoire_accompli):
    for interet in centre_interets:
        ...
        if cours[5] == 0 or option_obligatoire_accompli:
            if wordMatch(interet, str(cours[3])) or wordMatch(interet, str(cours[4])):
                return True
    return False
...
def proposition_cours_BD(no_immatriculation):
    ...
    cursor.execute(
        "SELECT cours.id_cours, nom_cours, type, periode, jour, heure_debut, heure_fin, nom_batiment, num_salle FROM cours, horaires, horairesalle, salle, batiment WHERE cours.id_cours = horaires.id_cours AND horaires.id_horaires = horairesalle.id_horaires AND horairesalle.id_salle = salle.id_salle AND salle.id_batiment = batiment.id_batiment AND obligatoire = " + str(
            periode))
    ...
    for cours in cours_proposees:
        cursor.execute(
            "SELECT cours.id_cours, nom_cours, type, periode, jour, heure_debut, heure_fin, nom_batiment, num_salle FROM cours, horaires, horairesalle, salle, batiment WHERE cours.id_cours = horaires.id_cours AND horaires.id_horaires = horairesalle.id_horaires AND horairesalle.id_salle = salle.id_salle AND salle.id_batiment = batiment.id_batiment AND cours.id_cours = \"" +
            cours[0] + "\"")
